...
//                return $userWithdrawHistories;
    ...
    public function statement($id)
    {
        ...
        $deposits = DB::table('userdeposits')->where('user_id',$id)->latest()->get();
        
        $withdraws = DB::table('userwithdraws')->where('user_id',$id)->latest()->get();
        
        foreach($deposits as $deposit)
        {
            array_push($data, $deposit);
        }
        
        foreach($withdraws as $withdraw)
        {
            array_push($data, $withdraw);
        }
        
        $collection = collect($data);
        
        $sort = $collection->sortByDesc(function ($element) {
            return  $element->created_at;
        });
        
        
        return view('backend.statement',compact('sort'));   
